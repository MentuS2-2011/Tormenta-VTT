...
export default function App() {
  ...
  /* Busca perfil — com retry caso o insert ainda não tenha terminado */
  async function fetchProfile(userId, attempt = 0) {
    ...
    if (data) {
      ...
      // Perfil ainda não existe — aguarda e tenta de novo (max 5x)
      setTimeout(() => fetchProfile(userId, attempt + 1), 800)
    } else {
      // Sem perfil mesmo após retries — usuário logado sem perfil
      setProfile(null)
      setLoading(false)
    }
  }
...
function AppLoader() {
  return (
    <div style={{
      ...
      <svg width="52" height="52" viewBox="0 0 44 44" fill="none" aria-label="Carregando">
...
function Placeholder({ page }) {
  return (
    <div className="page-centered" style={{ flexDirection: 'column', gap: '1rem', textAlign: 'center' }}>
      ...
      <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--clr-text-muted)', fontSize: '14px', letterSpacing: '0.2em', textTransform: 'uppercase' }}>
        Em construção
      </h2>
      <p style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 700, color: 'var(--clr-red-light)' }}>
        {page}
      </p>
      <p style={{ fontStyle: 'italic', color: 'var(--clr-text-muted)' }}>
        Esta página ainda será forjada.
      </p>
    </div>
  )
}
...
function NotFound() {
  return (
    <div className="page-centered" style={{ flexDirection: 'column', gap: '1rem', textAlign: 'center' }}>
      ...
      <p style={{ fontFamily: 'var(--font-display)', fontSize: '80px', fontWeight: 700, color: 'var(--clr-red)', lineHeight: 1, opacity: 0.3 }}>
        404
      </p>
      <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--clr-text)' }}>
        Página não encontrada
      </h2>
      <p style={{ fontStyle: 'italic', color: 'var(--clr-text-muted)' }}>
        Esta rota não existe em Arton.
      </p>
    </div>
  )
}